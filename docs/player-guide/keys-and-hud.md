...
| `key.mxt.flight_descend` | `X` | 飞行下降 | **驾驶飞行法器时的下降键**：按住就往下飞（不是一次性的开关）。原版的潜行是「下坐骑」，所以下降单独占一把键；前后左右是否跟随视线由服务端配置「飞行 → 朝视线方向飞行」决定。 |
...
轮盘是技能与灵气**唯一的触发入口**（法器能力就是技能），由**主盘 + 从盘**组成，格子用一套连续编号串起来：

- **主盘**就是你自己摆的那 12 格（编号 `0..11`）；**每次按 `R` 打开都回到它**。
- **从盘**不能摆也不需要摆：**主手物品**、**副手物品**、**法器**，内容是这三处装备**此刻**给你的主动技能，**加上它们作为法器声明的技能**（储物这类；飞行不在这里——它是功法授予的御器之术）。格子从 `12` 起接着排下去。换掉手里的剑、把法器摘下来，那几页当场就没了；装上去就都回来。
...
- **用掉一个技能 = 施放一次**（`mxt:active` 的主动技能才会出现在轮盘上，需要按键的 `mxt:flight_control` / `mxt:storage` 也各占一格）；**用掉一个灵气 = 发射一发灵力射线**；**用掉一个需要按键的技能 = 它做它该做的事**（御器之术翻一下：开＝从主手、其次副手取一件飞行法器起飞，关＝落剑并把那件法器还给你；储物则打开它自己的箱子）。消耗、冷却、施法时间、条件与效果**全部由服务端判定**，客户端只报告"用掉了哪个来源的哪一项"；服务端还会核对"这一项确实来自那个来源"。开关是开是关也由服务端说了算：你按一下，它读一遍现在的状态再决定，所以画面上的状态与实际一定一致。
...
- **屏幕左侧、竖直居中有一块「轮盘格」**：**永远是 4 列，行数随内容的格子数往下长**，画的是**整张轮盘的一览**——主盘和从盘的格子都在里面，但**只有主盘画空格子**（那 12 个空框是你自己摆的布局），从盘只画它真正有的技能（3 个技能就是 3 格，不会补一串空框）。格子里是图标或名字开头、底边一条色条（**技能金 / 灵气蓝；需要按键的技能按状态取色：开关开=绿、关=灰，一次性的是紫**）；**冷却中的格子按原版物品冷却那样压一层白幕**（盖住图标的剩余比例、随时间从上往下退），其它原因不可用时才压暗，**编号此刻代表的那一格是金色边框**，而**轮盘永远有一个选中的格子**，所以金框一定落在有内容的那一格上（整张轮盘一格都没有时轮盘根本不开、也就没有金框）、空格子只有空框。它显示的就是"现在按 `V` 会放什么"，也能一眼看到所有格里还放了什么；**块的上方不写任何字**——它是拿来看的，翻到哪一页由轮盘自己说。它和别的 HUD 元素一样，可以在 HUD 布局编辑器（默认右 `Shift`）里拖动、用 `/hud wheel.selection reset` 复位（**编辑器里目前只能移动，隐藏还没有开关**），尺寸会自己跟着页数长；如果你在旧版本里摆过原来那一格，用 `/hud wheel.selection reset` 或编辑器里的复位就能回到新的默认位置。
...
- **按了没生效会有一句动作栏提示**：**整张轮盘一格有内容的都没有**时按 `R`（或按 `V`）会说「轮盘上还没有任何条目」——这也是"没有目标"的**唯一**一种情况，因为轮盘只要有一格内容就永远有一个选中的格子。**指针指着的那一格是空的**时按 `V` 什么都不说——那一格在「轮盘格」上本来就是空框，轮盘也不会关，你的选择还留在原处。**服务端拒绝时会说明原因**：技能管线不肯施放时动作栏会报出来，接的是**同一份原因表**——「使用失败：<原因>」（需要按键的技能按不动时）与「施放失败：<原因>」（普通施放）后面跟的都是灵根与元素不符 / 某门资源不足（会点名是哪一门）/ 还在冷却 / 条件不满足 / 次数用完 / 没有权限 / 数值配置有误 / 状态已经是这样 / 这件法器不认你 / 需要一件承载它的物品 / 现在骑不上去……；这一项已经不在那个来源上时报「轮盘上的这一项已经失效了」，灵气发不出去时报「灵气没能发射出去」。日志里同时留一条 info，带原因名与缺的那门资源。有别的界面开着（聊天栏、物品栏……）时按键都不提示：那时你在打字或翻背包，不是在用轮盘。
- 轮盘开着的时候**角色会停下**（原版任何界面都会松开移动键），松开按键后照常走动，不用重新按方向键。
- 数字键 `1`–`9` 不再被本模组拦截，原版快捷栏一切照旧。

**需要按键的技能。** 有一类技能**必须按键才发动**，它们和其它条目一样落在轮盘上——今天有三样，跟着承载它的东西走：

- **施放**：已经学会的主动技（`mxt:active`）按一下就是施放一次，和以前一样。
- **飞行开关**：学会**御器之术**（`mxt:flight_control`，通常由功法授予）之后，那一页就多出一格「御器之术」，**按 `V` 从主手、其次副手取一件会飞的法器起剑，再按一次落剑**（法器原样回到背包），不需要另设快捷键；格子**底色绿＝开着、灰＝关着**，tooltip 写「状态：已开启 / 已关闭」。飞行中：`跳跃`上升、**下降键（默认 `X`）下沉**、`疾跑`给 1.5 倍水平速度，前后左右**默认沿视线方向**（服务端配置「飞行 → 朝视线方向飞行」，默认开；关掉后只在水平面上飞），原版的**潜行＝下坐骑**照旧（按 `Shift` 落剑）。这一趟只属于起剑的人：**你下线、死亡或被挤出 0 号位，载具立刻消失**（法器回到背包，主人不在线就落在原地），同车的乘客一起被放下——副驾驶不会接手。
- **储物**：法器声明了储物格数时，那一页会多出一格「储物」，**按 `V` 打开这件法器自己的箱子**（窗口标题是技能名，标准箱子界面：一行九格、最多六行）；关掉窗口什么都不留下，所以那一格是**紫色**的、没有状态行。

格子里写的是**技能名**（「御器之术」「储物」）而不是法器的图标：同一件法器可能两样都有，画同一个图标就分不出谁是谁；**是哪件法器**在 tooltip 里写着。这两个格子也能被你**钉到主盘**上：配置界面右边那一池里就有它们，钉上去之后只要那件法器还在**双手或法器槽**里就一直好用（放进背包不算"在身上"）；要认主的法器（`require_owner`）在认主之前那一格会画暗。储物窗口开着时，**法器一旦离开你身上（丢掉、被拿走）窗口会自己关掉**，免得往一个不在手里的东西上写东西。

**轮盘上放什么由玩家自己定。** 聊天栏打 `/wheel`，或按「打开轮盘配置」（`key.mxt.wheel_configuration`，默认未绑定）打开**轮盘配置界面**（两者都是纯客户端的，不发往服务端）：

- 上一排是两个可选项池——**左边 6 列是能发射的灵气，右边 6 列是已学会的技能与身上法器声明的技能**，各自滚动；悬停任一格显示它的 tooltip（类型 + 名字 + 消耗 / 冷却 / 施法时间 / 元素，或每次发射量 / 当前值 / 元素，或需要按键的技能的状态与是哪件法器）。
- 下一排是**共用的 12 格**，就是**主盘**的 12 格（**`1` 在正上方，顺时针**）。左键点池里一个条目选中它，再点某一格放进去；不选中条目直接点格子则清空该格。已填格子的底边颜色标明类型（灵气蓝、技能金；需要按键的技能按开 / 关取绿 / 灰、一次性取紫）。格子和环上的扇区一样：有图标画图标，没有就画名字开头，**放不下的部分在悬停 tooltip 里**。
- **只有主盘在这里编辑**：从盘的内容由随身装备与手里的御兽铃决定，站在界面里看的和在战斗里看的不是同一份，所以界面不预览它们（标题栏那一行写着「从盘（主手 / 副手 / 法器 / 契约）自动生成」）。
...
flowchart TD
    ...
    F --> G["技能施放一次、灵气发射一发，或需要按键的技能做它的事"]
    F --> H["只回一个原因，什么都不改"]
    G --> I["附件同步回客户端<br/>资源条、轮盘冷却照着画"]
```

条目契约是 `WheelMenuEntry`（`kind` / `id` / `title` / `icon` / `tooltip` / `cooldown` / `usable` / `onSelected`），技能与灵气各一个实现（需要按键的技能也是技能，不再单列一类）；Java 侧怎么接见 [客户端轮盘](../java/wheel.md)。
...
**现在可以拖的是资源条的左右两列与轮盘的「轮盘格」那一块**——屏幕中心两侧那两条竖排的条，每列是一个整体；轮盘格默认贴在屏幕左边、竖直居中，冷却直接画在格子上（一层随时间往下退的白幕）。资源本身显示在左列还是右列由数据包声明（`resource.bars` 的 `anchor`），但**列摆在哪里是玩家自己的设置**：把两列拖开可以避开别的模组的 HUD，或者把右列挪到中间看目标身上的资源。目标与 Boss 的覆盖条钉在准心所指的实体上，不参与拖动。编辑器里背景**不做模糊**（否则要摆的元素自己也糊了）。
