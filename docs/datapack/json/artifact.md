...
**法器技能（`ToggableArtifactAbility`）。** 判据只有一句：**凡是要按键才发动的都算技能、都进轮盘**。`abilities` 里凡是实现这个接口的条目都是这样的东西——实现它就等于声明"把这一项放进轮盘"（设计见 `research/32_法器开关与轮盘接线设计.md`）。今天有两个，都是固有类型、都不用新字段：`mxt:flight`（开关：开＝起剑、关＝落剑）与 `mxt:storage`（一次性：打开储物箱，没有状态）。约定：

- 它出现在**声明它的那张从盘**上（主手 / 副手 / Curios，排在技能之后），也可以被玩家钉到主盘——两处都读"那件法器此刻在不在双手或 Curios 槽里"，什么也不存。
- 轮盘条目的身份是**法器定义 id + 能力 key**（写成 `命名空间:路径/key`，如 `mxt_test:bound_sword/storage`），所以**一件法器可以有好几个技能，一个 key 一个**：同一把剑既能飞又能储物；同一个 key 写两次由加载期拒绝。
- **状态不是数据包字段**，归实现自己管（飞行读的是玩家那份飞行状态，储物没有状态）：轮盘那一格只报告状态（开关底色绿＝开、灰＝关，一次性的是紫），按下去由**服务端**问一遍实现再决定做什么。因此这些格子会出现在玩家轮盘上，是写下 `mxt:flight` / `mxt:storage` 的自然结果，不需要额外字段。
- 储物那一格打开的是**原版箱子界面**（工具条与窗口都复用原版容器菜单），格数按 9 向上取整、最多 54。

**还没做的部分**：精炼台仍未接入（`refine` 只有战利品函数、长按认主与脚本能用）。飞行与储物都已有玩家入口：轮盘上那两格（另有一条带 `enabled` 字段的 `FlightToggleC2SPayload` 通道，客户端目前不使用它）。
