...
秘境定义是**一份模板**，不是某个固定维度：每次进入都会按它开出一份**实例维度**，实例的身份就是那个维度键。维度键固定为 `<定义命名空间>:realm/<定义路径>/<序号>`，序号从 `0` 开始——**即使这份定义只能开出一份实例，序号也在**（`example:realm/trial_realm/0`）。每份实例的地形因此天然隔离在 `dimensions/<命名空间>/realm/<路径>/<序号>/` 下，灵气区域也能直接按这个 id 命中（见下面的「灵气整合」）。
...
| `exit_condition` | `EntityCondition` | `mxt:always_true` | **只约束主动离开**（令牌右键、`/mxt realm_instance exit`）；过期与强制送回不看它，否则数据包能把玩家永久锁在秘境里。 |
...
| `mxt:template` | `template`（`[A-Za-z0-9_-]+`）、`stem` | 从存档目录 `<服务器目录>/mxt_realm/<template>/` 复制 `region`、`entities`、`poi` 后再加载；手工搭好的地图走这条。 |
...
- 实体条件 [mxt:in_realm_instance](/datapack/types/condition/entity_condition_types) 判定"我是否在某份实例里"，`role` 取 `any`（在里面）、`owner`（我是主人）、`guest`（在里面但不是主人）；可选 `definition` 字段把判定限定到某个定义或标签。
- 公式变量 `realm_instance_is_owner`（`1` 或 `0`）、`realm_instance_members`、`realm_instance_limit`、`realm_instance_elapsed`、`realm_instance_duration`、`realm_instance_index`，可在进出条件与进出行为里读。不在任何实例里时这些名字按"无法提供"处理（开发环境会报日志）。名字带 `realm_instance_` 前缀是因为 `realm` 已经被境界占用了，见[公式变量](/datapack/types/formula_variables)。
...
实例维度是运行时维度，没有自己的 `LevelStem` 注册项，所以 [aura_zone](/datapack/json/aura_zone) 的 `dimensions` 除了直接写实例维度 id（`<命名空间>:realm/<路径>/<序号>`），还可以写该实例所用的 `stem`（如 `minecraft:the_end`）或定义 id——两者都会命中它的**所有份**。"末地型秘境统统是死灵气"这类规则因此只需要一条区域。
...
**删除定义前先清理实例。** 实例记录用注册表 holder 持久化定义引用，所以如果删掉一个**还有实例存在**的秘境定义，下次启动时那份记录会读不出来，它的地形会被当成遗留数据清掉。先 `/mxt realm_instance destroy <维度>`，再删定义。

失败码会出现在 `item.mxt.realm_token.enter_failed` 的提示里（除非定义自带 `*_denied_message`）：`DISABLED`（定义被 `mxt:disabled` 停用）、`ALREADY_TRAVELLING`、`CONDITION_NOT_MET`、`NO_FREE_INSTANCE`、`MISSING_STRUCTURE`、`GENERATION_FAILED`、`MISSING_DIMENSION`（只有 `mxt:existing` 会缺维度）、`FULL`、`CANCELLED`（`EnterPre` 事件被取消）、`EXIT_DENIED`、`NOT_TRAVELLING`、`MISSING_ORIGIN`。
