...
`quality` 定义品质内容，**顺序、默认档与升级路径由 [quality_chain](./quality_chain.md) 决定**：物品通过 `quality_chain` 引用一条链，解析出的档必须在链上，否则不能使用。`currency` 为物品定义货币价值，`unavailable_when` 是包含 `condition` 和 `reason` 的 ItemCondition 数组。
