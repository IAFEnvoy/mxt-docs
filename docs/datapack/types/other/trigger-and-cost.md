...
`Cost` 先检查，再一次性扣除。五种写法、通道规则与全有或全无语义见[共享数据类型 · `Cost`](../shared_data_types.md#cost)。

| `type` | 字段 | 说明 |
| --- | --- | --- |
| `mxt:resource` | `resource`、`amount` | 从付款者的数值账户中消耗一个数据包数值 |
| `mxt:aura` | `aura`、`amount` | 按灵气身份消耗：由付款者支付时扣**这门灵气所度量的那个数值**，由共享灵气池或方块存量支付时扣这门灵气本身 |
| `mxt:item` | `items`、`amount` | 从玩家背包中消耗匹配的物品 |
| `mxt:js` | `id`、`params?` | 由服务端脚本回调检查并支付；需要玩家，并且在其它通道全部付完之后最后运行 |
...
| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `aura` | `Holder<aura>` | **必填** | 要消耗的灵气身份 |
| `amount` | `NumberProvider` | **必填** | 要消耗的数量；必须求值为有限正数。付款者支付时从数值账户扣该灵气所度量的那个数值；从共享灵气池或方块存量支付时扣这门灵气本身（见共享数据类型的通道表） |
...
脚本 Cost 在服务端先检查、后支付，并且需要一个玩家。回调收到的是支付者与 `params`，而不是技能的公式上下文，因为 `Cost` 只用付款者求值。

付款者是**活着的实体**（玩家、生物、召唤物都算），不一定是玩家：`mxt:item` 需要玩家背包，付款者不是玩家（或阵法没有阵主）时就是**付不出**而不是报错；`mxt:js` 需要玩家，并且**在所有其它通道都付完之后最后运行**——脚本消耗不做暂存，所以脚本必须自己对它保持幂等。

整份数组是**全有或全无**的：任何一项付不出，就什么都不扣。同一数组里两项指向同一个存储（同一个数值写两次，或同一门灵气写两次）会让定义**加载失败**；而一个 `mxt:resource` 与一个用该数值度量的 `mxt:aura` 会把金额相加，那不是错误。写错的条目也**不再被静默丢弃**，它会让定义加载失败。
