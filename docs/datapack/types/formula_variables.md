...
- 秘境实例：`realm_instance_members`、`realm_instance_limit`（不限时为 `-1`）、`realm_instance_elapsed`、`realm_instance_duration`、`realm_instance_index`、`realm_instance_is_owner`（`1` 或 `0`）。只要公式是对某份[秘境实例](../json/realm_instance.md)的成员求值的就能读，最典型的是定义自己的进出条件与进出行为。不在任何秘境实例里时这些名字无法提供，按错误处理而不是静默为 `0`。前缀带 `realm_instance_` 是因为 `realm` 已经被境界占用了。
