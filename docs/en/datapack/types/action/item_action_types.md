...
| `mxt:charge_artifact` | `amount`, `capacity` | Adds spirit energy to an artifact stack, measured against the capacity its `item_archetype` declares; the declared `capacity` is only the fallback for a stack whose archetype is missing, no longer resolves or declares no positive finite value. |
