...
|-------------|------|---------|-------------|
| `element` | Entity action | **required** | The action this entry runs when it is picked. |
...
| `mxt:damage` | `amount` | Applies damage to the entity. It is **unowned**, so no spirit-root element relation applies and no attacker is credited: the right shape for recoil, pill toxicity and environmental ticks. When it lands on somebody other than the caster, the caster is credited instead. See the [damage pipeline](../../damage.md). |
