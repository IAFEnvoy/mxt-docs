...
| `mxt:realm` | `realm`, `comparison?`, `min_minor_stage?` | Compares the entity's [realm](../../json/realm_stage.md) stage against `realm`, using `exact` (the default), `at_least` or `at_most`. The optional `min_minor_stage` (`0`-based, the same numbering as the `minor_stage` formula variable) additionally requires the highest minor stage the body has reached **in that realm** to be at least that — it reads the record that only ever grows, so a gate like "has reached layer 500 of qi refining" **stays true** after breaking through; pair it with `comparison: "exact"` to mean "is in that layer or beyond right now". |
...
| `mxt:formation_member` | — | Passes when the entity owns any registered [formation](../../json/formation.md) in the current level (being anywhere on the **ownership list** counts). |
| `mxt:formation_owner` | — | Passes when the entity is **one of** the owners of the formation currently being evaluated (ownership is a set of UUIDs, and any of them counts); outside a formation context it is `false`. |
| `mxt:formation_ally` | — | Passes when **any** owner of the formation currently being evaluated treats the entity as an ally. |
