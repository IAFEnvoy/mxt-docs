...
**Not built yet**: the refining table is still missing (refining is reachable only from the loot function, the long press and scripts). Flight and storage both have a player-facing entry now: those two wheel cells (the flying one granted by a technique, the storage one by the artifact).
