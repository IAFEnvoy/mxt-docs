...
| `mxt:flight` | `speed` (required `NumberProvider`), `costs` (`ResourceCost[]`, default `[]`) | Flight: `speed` is the mount's flying speed and `costs` is what is paid per tick. **At most one such entry per definition.** |
| `mxt:storage` | `slots` (required `NumberProvider`) | Built-in storage slots. **At most one such entry per definition.** |
...
**Not built yet**: flight has no player-facing entry today (the client never sends the flight-toggle packet) and there is no storage UI, so what a `mxt:flight` or `mxt:storage` entry says only reaches as far as "the server knows this artifact offers it".
