...
| `rarity` | String | `common` | Rarity marker; the info panel and `/mxt identity root list` show the raw text, and the `mxt.rarity.<rarity>` translation is used when one exists. |
