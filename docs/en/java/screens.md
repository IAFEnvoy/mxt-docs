...
How a translation key is spelled is decided in one place, `com.iafenvoy.mxt.util.DefinitionText`: the category defaults to the registry's own path and the registry namespace is always `mxt`, and the few whose category is not the registry path (`mxt:item_quality` has always been translated as `quality`) are declared once in its internal `CATEGORIES`. With a `Holder` / `ResourceKey` already in hand, call `DefinitionText.name(holder)` directly; only when all you have is a bare `Identifier` do you pass the category in as an argument (`DefinitionText.name(id, "resource")`).
