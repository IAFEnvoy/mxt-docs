...
| `DefinitionText` | Turns a datapack definition into a name: with a `Holder` or a `ResourceKey` in hand, call `name(holder)` directly, and the category defaults to the registry's own path while the registry namespace is always `mxt` (`mxt:fire` in `mxt:aura` looks up `aura.mxt.mxt.fire`). The few whose category is not the registry path are declared once in its internal `CATEGORIES` (such as `mxt:item_quality` → `quality`, where `example:refined` looks up `quality.mxt.example.refined`), so one definition can never have two names in a tooltip and in the item picker. The definitions of 18 registries such as `resource` and `aura` carry `name` / `description`, which are read from the fields themselves; an omitted field is filled in by `ContextNameCodec` while the entry loads with the **same** key (`description` appends `.description`). |
