...
| **Hotbar entries** | Add a client-side hotbar entry by implementing `HotbarEntry`; see [Hotbar Entries](./hotbar.md). |

## Next Steps

- [Public API](./api.md) — the runtime services an addon calls into, from `AuraService` to `DefinitionText`.
- [Interfaces](./interfaces.md) — `AuraAccess`, `ItemAuraAccess`, `UseItemAuraAccess`, `TooltipAppender`, `Cost` and `HotbarEntry`.
...
- [Hotbar Entries](./hotbar.md) — how to add an entry to the client hotbar.
