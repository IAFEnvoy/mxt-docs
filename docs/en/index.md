...
| Wheel and Character UI          | ✅  | Abilities and spirit power share one wheel: the main wheel's twelve cells are yours to arrange, and the pages behind it come from what you carry, opening another page when one is full. Hold `R` and point at a cell to cast or fire.              |
...
| Artifacts                       | 🚧 | Existing items become artifacts through `artifact`: `items` claims them, `spirit_capacity` names a ceiling per aura, and `abilities` grants active and passive skills, flight and storage. Refining, flight entry and storage UI are still missing. |
