...
| Wheel and Character UI          | ✅  | Abilities, spirit power and artifact skills share one wheel: the main wheel's twelve cells are yours to arrange, and the pages behind it come from what you carry, opening another page when one is full. Hold `R` and point at a cell to use it.              |
...
| Artifacts                       | 🚧 | Existing items become artifacts through `artifact`: `items` claims them, `spirit_capacity` sets a per-aura ceiling, and `abilities` grants skills, flight and storage - flight and storage are wheel cells. Refining is still missing. |
