...
A secret realm's dimension id is derived from the definition and an index: `<definition namespace>:secret_realm/<definition path>/<index>`, counting from 0 — so one template can open many worlds that never touch each other.
...
- `owned: true`: when the owner logs off and everyone leaves, the world merely **goes dormant** — terrain, blocks, builds and contents stay, and the next visit reopens it under the same seed. It survives a server restart, but the member table is cleared by one (`loaded=false` in `/mxt secret_realm list` is exactly that).
...
- **Biome-bound aura zones do not follow you in**: an `mxt:void` secret realm is `minecraft:the_void`, so a zone bound to `#minecraft:is_overworld` does not match. With no zone matching, the position falls back to the empty zone — a secret realm has **no aura by default**, and you have to bind one deliberately.
...
4. Walk around, `exit`, then inspect it with `/mxt secret_realm info <dimension>`; enter an `owned: true` instance again and you should be back in the same terrain.
...
| After a restart a claimed secret realm is `loaded=false` and players cannot return | That is dormancy, not loss: the next visitor reopens it under the same seed. The member table is cleared by the restart, so players it considers orphaned are sent back to where they entered from when they log in. |
...
| After editing the definition nothing loads | A secret realm definition is a registry entry, so a decode error **stops the world from loading**; read the last codec error in the log. |
| Structures did not appear | A `chance` below 1 rolls the dice once per instance, and an `integrity` below 1 leaves random holes. |

## Next

- [secret_realm](../datapack/json/secret_realm.md) — the full field list and every generation type.
- [aura_zone](../datapack/json/aura_zone.md) — binding aura to a secret realm dimension.
- [formation](../datapack/json/formation.md) — raising an array inside a secret realm.
- [Rifts](../player-guide/rift.md) — connecting a secret realm to the overworld.
