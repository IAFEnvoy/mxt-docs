...
| Field | Effect |
| --- | --- |
| `timeline` | **Required**, the timeline the run copies into its attachment and then walks with a cursor, at least one beat. |
| `windup` | Ticks to idle through before the first beat starts. |
| `success_action` | Runs once the cursor walks off the end of the timeline; default `mxt:no_op`. |
| `fail_action` | Runs when a beat cannot continue; default `mxt:no_op`. |

There are four kinds of beat:
...
| `mxt:wait_for` | `condition` (`EntityCondition`, **required**), `timeout` (ticks, optional), `on_timeout` (`fail` by default, or `finish`) | Checked once per tick; finishes when it passes. With a `timeout` it is a **deadline**, and when it runs out the run fails or moves on according to `on_timeout`. |
| `mxt:branch` | `condition` (`EntityCondition`, **required**), `if_true` / `if_false` (absolute indices, optional) | Moves the run's **cursor** to another beat and finishes in the same tick; a branch that is not written simply advances one beat. |
...
## Step 3 — Splitting Into Two Branches

`mxt:branch` keeps a timeline from having to be a straight line: in the same tick it moves the cursor to the beat named by `if_true` or `if_false` (absolute indices, `0`-based), and with neither written it simply advances one beat. The timeline below first waits with a deadline for the player to stand under open sky (`timeout: 600` ticks, failing the run if they do not), then splits on health — a low-health player is sent by `if_true: 3` straight to the 4th beat, skipping the lighter bolt on the 3rd:
...
An out-of-range index is refused **at start**, so a branch pointing at the wrong beat cannot surface only after the player has paid for the breakthrough; **backward jumps are allowed** too (nothing stops a "go back and take another round while the health is low" loop, so the condition has to be able to settle on its own).
...
## Step 4 — Striking With Lightning
...
## Step 6 — Hooking It to a Breakthrough
...
| `start` says a beat cannot run now | The pre-start validation failed, most often an `mxt:idle` whose `duration` cannot be resolved (a formula using a variable the empty context does not provide); it can also be an `mxt:wait_for` whose `timeout` does not resolve to a positive number, or an `mxt:branch` whose `if_true` / `if_false` is outside the timeline. |
| The tribulation is stuck on one beat | That beat's `mxt:wait_for` condition is unreachable and it has no `timeout` — so it neither advances nor fails; give it a deadline (a `timeout` plus `on_timeout`) to give it an end. |
| `mxt:branch` jumped to the wrong place | Indices are **absolute** (counted from beat 1 of the copied timeline, `0`-based), not an offset from the current beat; an out-of-range value is refused at start. |
