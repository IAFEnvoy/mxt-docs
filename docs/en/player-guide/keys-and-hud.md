...
The wheel is the **only** way an ability, a spirit power or an **artifact skill** is triggered, and it is a **main wheel plus pages read from what you carry**, all strung together by one continuous cell numbering:

- **The main wheel** is the twelve cells you arrange yourself (numbers `0..11`); **opening with `R` always returns to it**.
- **The pages behind it** are ones you neither arrange nor need to: **main hand**, **off hand**, **artifacts**. Their contents are the active abilities those three places grant you **right now**, **plus the skills they declare as artifacts** (flight and storage), numbered from `12` on. Swap the sword in your hand or take an artifact off and those pages are gone on the spot; equip one and its abilities and artifact skills are back.
...
- **Spending an ability casts it once** (only `mxt:active` abilities appear on the wheel); **spending a spirit power fires one burst**; **spending an artifact skill does what that artifact does** (a flight switch flips: on takes off, off lands; the storage opens its own box). Costs, cooldowns, cast times, conditions and effects are **all decided by the server** - the client only reports which entry of which source was spent, and the server checks that the source really holds it. Whether a switch is on is the server's answer too: you press once, it reads the state and decides, so the cell and the world can never disagree.
...
- **A key that could do nothing answers on the action bar**: with **nothing in any cell of the whole wheel**, `R` (or `V`) says "Nothing is on the wheel yet" - and that is the **only** case of "no target" left, because a wheel with anything in it always has a selected cell. **`V` on a cell with nothing in it** says nothing at all - that cell is already an empty frame in the "Wheel Grid", the wheel does not close, and your choice stays where it was. **A refusal from the server does say why**: when the ability pipeline will not cast it (roots do not match its element, not enough of a resource, still on cooldown, conditions unmet, ...) the action bar says "Cast failed: reason" and names the resource that ran short; an entry that source no longer holds says "That entry is no longer on the wheel"; a burst that does not go off says "The burst did not go off"; a switch that will not move says "Use failed: reason" (not on you / not yours / already in that state / cannot be used now). With another screen open (chat, inventory, ...) the keys say nothing: you are typing or looking through your bag, not asking the wheel for something.
...
**Artifact skills.** Some capabilities **need a key to fire**, and they put themselves on the wheel - there are two today, and both travel with the artifact:

- **Flight switch**: while you hold or wear the artifact that can fly, the matching page gains a "Flight" cell - **press `V` to take off, press it again to land**, with no extra keybind; its strip is **green while it is on and grey while it is off**, and the tooltip says "State: on / off".
- **Storage**: when the artifact declares storage slots, that page gains a "Storage" cell - **press `V` to open the artifact's own box** (the window is titled after the artifact and is the standard chest layout: nine to a row, at most six rows). Closing it leaves nothing behind, so that cell is **violet** and has no state line.

The cell writes the **capability's name** ("Flight", "Storage") rather than the artifact's icon: one artifact may offer both, and two identical sword icons would say nothing about which is which - **which artifact** it belongs to is in the tooltip. Either cell can also be **pinned to the main wheel**: they are in the right-hand pool of the editor, and once pinned a cell keeps working as long as the artifact is in **either hand or a Curios slot** (a backpack is not "on you"). An artifact that requires an owner (`require_owner`) is drawn dim until you claim it. While a storage window is open, **the moment the artifact leaves you (dropped, taken) the window closes itself**, so nothing is ever written to something you are not holding.
...
flowchart TD
    ...
    F --> G["An ability casts once, one burst fires, or an artifact skill does its thing"]
    F --> H["Only a reason comes back, nothing changes"]
    G --> I["Attachments sync back<br/>bars and wheel cooldowns follow"]
```

The entry contract is `WheelMenuEntry` (`kind` / `id` / `title` / `icon` / `tooltip` / `cooldown` / `usable` / `onSelected`), implemented once for abilities, once for spirit powers and once for artifact skills; see the [Java wheel page](../java/wheel.md) for how to plug something in.
