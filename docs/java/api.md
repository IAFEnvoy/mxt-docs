...
这两件事一起讲，因为最容易混：**灵气（`aura`）是身份，资源（`resource`）是数值**。字段怎么写见 [aura](/datapack/json/aura) 与 [resource](/datapack/json/resource)，"某坐标的灵气到底怎么算出来"见[灵气计算](/technical/aura)。
...
- **一次查询是"本刻快照"**：结果按 `(坐标, 游戏刻)` 记忆、整表在每个 tick 末尾丢弃（`AuraQueryCache`），所以 `consume` / `change` 之后在**同一刻**再查，读到的仍是改动前的值（新激活的阵法同理，下一 tick 才看得到）——要立刻拿到新值，就用这次调用的返回值自己记账。详见[灵气计算](/technical/aura)。
...
包 `com.iafenvoy.mxt.runtime.damage`。**这一击的唯一结算管线**：成形在攻击方（`outgoing`），减免在受击方（`incoming`）。两层为什么必须分开、数据包里怎么写元素，见[伤害系统](/technical/damage)。
...
### `FormationActionType` 与 `MxtFormationActionTypes` {#formationactiontype}
...
包 `com.iafenvoy.mxt.runtime.friend`。"**这个是不是我的人**"的唯一提问入口：友伤、护持豁免、AI 判断都走它。机制与名单命令见[敌我识别系统](/technical/identification)。
...
包 `com.iafenvoy.mxt.runtime.economy`。物品本位货币定义的**服务端读取 API**：只算价值、给兑换报价，**不扣费**——货币不是消耗。格式见 [currency](/datapack/json/currency)。
...
- **客户端只发意图**：新增交互时只传 id 与选择，服务端自己重新解析定义再判定，见[网络协议与服务端权威](/java/network)。
