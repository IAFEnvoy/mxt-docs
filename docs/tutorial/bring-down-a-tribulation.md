...
| 字段 | 作用 |
| --- | --- |
| `timeline` | **必填**，天劫要复制进附件、再用游标逐拍走完的时间线，至少一拍。 |
| `windup` | 启动前摇：第一拍开始之前先空转的 tick 数。 |
| `success_action` | 时间线走空之后执行，默认 `mxt:no_op`。 |
| `fail_action` | 某一拍报错、天劫无法继续时执行，默认 `mxt:no_op`。 |

节拍有四种，写一场天劫就是把它们排成一队：
...
| `mxt:wait_for` | `condition`（`EntityCondition`，必填）、`timeout`（ticks，可选）、`on_timeout`（`fail` 默认 / `finish`，可选） | 每 tick 求值一次，条件成立才结束这一拍；写了 `timeout` 就是**限期等待**，到点按 `on_timeout` 失败或直接过。 |
| `mxt:branch` | `condition`（`EntityCondition`，必填）、`if_true` / `if_false`（绝对下标，可选） | 按条件把**运行游标**移到另一拍，并在同一个 tick 结束；不写的分支照常前进一拍。 |

时间线是**运行游标**：启动时整条被复制进实体的附件，之后每 tick 消费游标所在那一拍，跑完就前进一拍。**游标是存下来的**（`mxt:branch` 可以把它移到任意一拍，也可以回跳），所以 `/reload` 改动定义**不会**影响一场已经在进行的天劫——时间线已经拷走了；而难度倍率与成败行为仍然从定义实时读取。`if_true` / `if_false` 的下标从**这条被复制的时间线的第一拍**算起（`0` 起），越界会在**启动时**被拒绝。

::: tip `mxt:wait_for` 可以限期
不写 `timeout` 时，条件永不成立，天劫就永远停在这一拍：不推进，也不失败——作者要保证条件可达（例如用 `mxt:exposed_to_sky` 这种迟早会真的条件）。

写了 `timeout`（ticks）就是**限期**：到点按 `on_timeout` 走——`fail`（默认）判失败，`finish` 直接过了这一拍。它是限期而不是这一拍的时长，所以只按 `timeout` 结算，**不乘 `difficulty_scale`、也不看环境灵气**：难度不该决定玩家有多少时间达标。`timeout` 解不出正数时整场启动被拒绝。
:::
...
**启动前每一拍都会被问一次「现在能不能跑」**，`mxt:idle` 的时长解不出来、`mxt:wait_for` 的 `timeout` 解不出来、`mxt:branch` 的下标越界时，整场启动直接被拒——坏定义不该在玩家已经付掉突破代价之后才出问题。

## 第 3 步 —— 分两条支线

`mxt:branch` 让时间线不必是直线：它在同一个 tick 里把游标搬到 `if_true` 或 `if_false` 指定的那一拍（绝对下标，`0` 起），两个都不写就照常前进一拍。下面这段先是限期等玩家站到天空下（`timeout: 600` 刻内没做到就判失败），然后按血量分流——血少的人被 `if_true: 3` 直接送到第 4 拍，让过第 3 拍那道较轻的雷：
...
下标越界在**启动时**就被拒绝，所以跳错位置的分支不会等到玩家付掉突破代价才出问题；**回跳也允许**（做一个「血量低就回去再挨一轮」的循环没有限制，条件要靠自己收得住）。
...
- **成功**：游标走出时间线末尾即执行 `success_action`。上面写的是补 50 点灵气——扛过雷劫，池子反而更满。
- **失败**：某一拍判定无法继续（`mxt:idle` 的现场丢了、节拍自己返回失败、`mxt:wait_for` 的 `timeout` 到点且 `on_timeout` 是 `fail`）时执行 `fail_action`。劫数**没有任何内置惩罚**——不掉境界、不扣修为，全部由你写在 `fail_action` 里。
...
| `start` 报「某个节拍现在跑不了」 | 启动前的逐拍校验失败，最常见的是 `mxt:idle` 的 `duration` 解不出来（公式用了空上下文里没有的变量），也可以是 `mxt:wait_for` 的 `timeout` 解不出正数、`mxt:branch` 的 `if_true` / `if_false` 超出时间线范围。 |
| 天劫永远停在某一拍 | 那一拍的 `mxt:wait_for` 条件不可达，而且没有写 `timeout`——不推进也不判失败；补一个 `timeout`（配合 `on_timeout`）就能给它一个尽头。 |
| `mxt:branch` 跳错了地方 | 下标是**绝对**的（从这条被复制的时间线的第 1 拍算起、`0` 起），不是相对当前拍的偏移；越界的值在启动时就被拒绝。 |
