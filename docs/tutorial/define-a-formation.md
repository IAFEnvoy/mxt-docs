...
| 字段 | 类型 | 默认 | 说明 |
| --- | --- | --- | --- |
| `activation_costs` | `List<ResourceCost>` | `[]` | 激活时一次性扣除，从**激活者**身上扣；任何一步失败都不消耗。 |
| `maintenance_costs` | `List<ResourceCost>` | `[]` | 每个周期扣一次，决定阵法能撑多久。 |
| `storage` | 对象 | 无（不启用） | 写了就必须有 `capacity`：`Map<aura, NumberProvider>`。 |

::: warning 费用列表会静默吞掉写错的条目

这两个列表用的是"容错列表"解码器：**写错的条目只打一条日志然后被丢掉**，不会让定义加载失败。也就是说 `{"id": "example:qi", "amont": 200}` 这种拼写错误的结果是**阵法免费**——它照样能激活，只是不花钱。

而 `structure` 与 `actions` 是严格列表，写错就直接加载失败。**改完阵法先看日志**，别只看"能不能激活"。
...
| 阵法能激活但从不花钱 | `activation_costs` / `maintenance_costs` 里的条目写错了**字段名**——容错解码只打日志就丢掉它。 |
